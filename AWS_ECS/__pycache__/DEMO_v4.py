...
def fillGPT(info, dict1):
    prompt = info + ": That is from my resume, when you put those values in this dictionary, make sure there are no quotes, double quotes, brackets, or parentheses, it should be all text only. DO NOT ELIMINATE ANY BULLET POINTS. Here's the dictionary: " + str(dict1)
    model = "gpt-3.5-turbo-1106"
    response = openai.ChatCompletion.create(model=model,messages=[{"role": "system", "content": "You are a helpful assistant."},
                                                                  {"role": "user", "content": prompt + "json"}], max_tokens=500, response_format = { "type": "json_object" })
